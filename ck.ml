...
let rec conjoint p =
  ...
  | pp :: suite ->
      ...
      else if pp=S(VassalAInfluencer) || pp=S(EtrangerAInfluencer) then "S'occuper de politique"
...
let rec chancelier p =
  ...
  | pp :: suite ->
      ...
      else if pp=S(EtrangerAInfluencer) then "Gérer les affaires étrangères"
...
let rec decision p =
  ...
  | pp :: suite ->
      ...
      else if pp=S(VassalAInfluencer) || pp=S(EtrangerAInfluencer) then "influence visée PUIS Politique PUIS Intrigue PUIS emprisonner si Sombres connaissances PUIS " ^ decision suite
...
let p = prios [  ] [ EtrangerHamecon ; Revenu  ] 2
